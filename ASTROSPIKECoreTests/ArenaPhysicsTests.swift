...
struct ArenaPhysicsTests {
    ...
    @Test("Safe landing survives while a high speed crash scores")
    func crashThresholdSeparatesLandingFromDeath() {
    ...
    @Test("A high-speed opponent impact destroys both ships and replays the rally")
    func highSpeedShipImpactReplaysRally() {
        var engine = SimulationEngine.testing()
        engine.state.ships[.cyan] = ShipState(position: SIMD2(-0.09, 0.40), velocity: SIMD2(8, 0), angle: 0)
        engine.state.ships[.orange] = ShipState(position: SIMD2(0.09, 0.40), velocity: SIMD2(-8, 0), angle: .pi)

        engine.step(inputs: [.cyan: .idle(tick: 0), .orange: .idle(tick: 0)])

        #expect(engine.state.ships[.cyan]!.isDestroyed)
        #expect(engine.state.ships[.orange]!.isDestroyed)
        #expect(engine.lastEvents == [.rallyReset])
